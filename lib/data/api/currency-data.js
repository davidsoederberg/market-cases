...
exports.currencyRate = (from, to) => new Promise((resolve, reject) => {
  const uri = `https://www.alphavantage.co/query?function=CURRENCY_EXCHANGE_RATE&from_currency=${from.toUpperCase()}&to_currency=${to.toUpperCase()}&apikey=${process.env.AV_KEY}`;
  request({ url: uri, json: true }, (error, response, body) => {
    ...
    if (!body[accessData]) {
      ...
      resolve(format.format(`${from}/${to}`, Number(body[accessData][accessRate])));
    }
  });
});
