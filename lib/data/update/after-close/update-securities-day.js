...
const test = require('debug')('b-rscase:test:cron:update-securities-day.js');
...
exports.update = async () => {
  try {
    ...
    await updateDatabase(securities, await Promise.all(data));
    debug(`${moment().format('YYYY/MM/DD HH:mm:ss')} - Update of daily prices done.`);
    test(`${moment().format('YYYY/MM/DD HH:mm:ss')} - Update of daily prices done.`);
  } catch (e) {
    debug(chalk.red(e));
    throw e;
  }
...
async function updateDatabase(securities, data) {
  for (const security of securities) {
