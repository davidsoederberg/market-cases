...
describe('api-securities-route testing', () => {
  ...
  describe('PUT /sec/:id', () => {
    it('should return status 200 and the new price should be added', (done) => {
      const newSec = new Security();
      newSec.name = 'Gold';
      newSec.startingPrice = 100;
      newSec.type = 2;
          ...
          chai.request(server)
            ...
            .end((err, res) => {
              if (err) {
                ...
                expect(res).to.be.status(200);
                expect(res.body.prices[0].price).to.be.equal(1337);
                done();
              }
            });
        });
    });
    it('should return status 400 because invalid id', (done) => {
      chai.request(server)
        .put('/api/sec/123')
        .send({ price: 1337 })
        .end((err, res) => {
    ...
    it('should return status 400 because no price provided', (done) => {
      const newSec = new Security();
      newSec.name = 'Gold';
      newSec.startingPrice = 100;
      newSec.type = 2;
