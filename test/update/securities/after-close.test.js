...
const secAfterClose = require('../../../lib/data/update/securities/after-close');
...
describe('securities after-close update', () => {
      ...
      .then(async () => {
        ...
        Security.findById(gold.id, (err, doc) => {
          if (err) {
            ...
            expect(doc.dayData[0].day).to.be.equal(moment().subtract(1, 'days').format('YYYY/MM/DD'));
          }
        });
        await secAfterClose.update();
        Security.findById(gold.id, (err, doc) => {
